	...
	if event is InputEventMouseButton:
		match event.button_index:
			BUTTON_RIGHT: # Only allows rotation if right click down
				var mode
				match event.pressed:
					true: mode = Input.MOUSE_MODE_CAPTURED
					false: mode = Input.MOUSE_MODE_VISIBLE
...
func _update_mouselook():
	...
	if Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		...
		pitch = clamp(pitch, -90 + rotation_degrees.x, 90 + rotation_degrees.x)
		
		rotate_y(deg2rad(-yaw))
		rotate_object_local(Vector3(1,0,0), deg2rad(-pitch))

func set_enabled(setting=true):
	enabled = setting
	if setting == false:
